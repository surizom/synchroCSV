...
public class Task {
  ...
  public Task(
      String name, int yearS, int monthS, int dayOfMonthS, int yearF, int monthF, int dayOfMonthF) {
    this.start = new Date(yearS, monthS, dayOfMonthS);
    this.finish = new Date(yearF, monthF, dayOfMonthF);
    this.name = name;
  }
  ...
  /**
   * On considère que deux tâches sont égales si elles ont même nom (sans prendre en compte la casse
   * ni les espaces)
   */
  @Override
   ...
   * Attention le programme ne prend en compte que des tâches de niveau 2 pour l'instant (de toute
   * façon les tâches de niveau 3 ça sert à rien)
   *
   * @return
   */
  public String[] toCSV() {
    String[] res = {
      this.name, this.start.toString(), this.finish.toString(), "2", this.getResources()
    };
    return res;
  }
}
